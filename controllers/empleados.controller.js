...
exports.update = function(req,res){
    ...
    Empleado.findByIdAndUpdate(req.params.id, {$set: empleado}, function(err){
        if (err){
            console.error(err),
            response.exito = false, 
            response.msg = "Errror modificando datos de empleados"
            res.json(response)
            return;
        }
        response.exito = true,
        response.msg = "empleado modificado"
        res.json(response)
    })
}
...
exports.remove = function(req,res){
    Empleado.findByIdAndRemove({_id:req.params.id}, function(err){
        if (err){
            console.error(err),
            response.exito = false, 
            response.msg = "Error eliminando el empleado"
            res.json(response)
            return;
        }
        response.exito = true,
        response.msg = "empleado eliminado satisfactoriamente"
        res.json(response)
    })
}
